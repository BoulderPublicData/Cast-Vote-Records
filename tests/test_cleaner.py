"""Tests for the City-of-Boulder cleaner."""
...
def test_detect_city_ballot_types_min_ballots_threshold_drops_aggregate(minimal_cvr_path):
    df = load_raw_cvr(minimal_cvr_path)
    # The DS-02 NaN-CvrNumber aggregate row has City of Boulder votes filled
...
def test_clean_drops_all_three_aggregate_conventions(minimal_cvr_path):
    df = load_raw_cvr(minimal_cvr_path)
    result = clean_city_cvr(df, election_key="test")
    assert result.n_raw_rows == 21
    # Sentinel-string row + NaN-CvrNumber row + NaN-TabulatorNum row = 3 dropped
    assert result.n_redacted_dropped == 3
    assert result.city_ballot_types == ("DS-01",)
    assert result.n_city_ballots == 12


def test_clean_drops_all_nan_choice_columns(minimal_cvr_path):
    df = load_raw_cvr(minimal_cvr_path)
    result = clean_city_cvr(df, election_key="test")
...
def test_clean_flattens_columns(minimal_cvr_path):
    df = load_raw_cvr(minimal_cvr_path)
    result = clean_city_cvr(df, election_key="test")
...
def test_clean_empty_result_when_no_city_ballots(minimal_cvr_path):
    df = load_raw_cvr(minimal_cvr_path)
